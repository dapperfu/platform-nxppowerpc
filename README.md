...
Development platform for the **NXP DEVKIT-MPC5744P** board (MPC5744P / e200z4) using PlatformIO and the PowerPC EABI VLE toolchain. Firmware is compiled and linked against **EWL** (`ewl_c9x_noio.specs`: `-lc99 -lm -lrt`) from `e200_ewl2` inside the downloaded toolchain tarball. An expanded tarball on disk is reference only; do not use an on-device S32DS directory.
